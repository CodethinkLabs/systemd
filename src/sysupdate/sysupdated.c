...
static int job_new(JobType type, Target *t, sd_bus_message *msg, JobComplete complete_cb, JobReady detach_cb,  Job **ret) {
...
static int target_method_list(sd_bus_message *msg, void *userdata, sd_bus_error *error) {
        ...
        r = job_new(JOB_LIST, t, msg, target_method_list_finish, NULL, &j);
        if (r < 0)
                return r;
...
static int target_method_describe(sd_bus_message *msg, void *userdata, sd_bus_error *error) {
        ...
        r = job_new(JOB_DESCRIBE, t, msg, target_method_describe_finish, NULL, &j);
        if (r < 0)
                return r;
...
static int target_method_check_new(sd_bus_message *msg, void *userdata, sd_bus_error *error) {
        ...
        r = job_new(JOB_CHECK_NEW, t, msg, target_method_check_new_finish, NULL, &j);
        if (r < 0)
                return r;
...
static int target_method_update(sd_bus_message *msg, void *userdata, sd_bus_error *error) {
        ...
        r = job_new(JOB_UPDATE, t, msg, target_method_update_finished_early, target_method_update_detach, &j);
        if (r < 0)
                return r;
...
static int target_method_vacuum(sd_bus_message *msg, void *userdata, sd_bus_error *error) {
        ...
        r = job_new(JOB_VACUUM, t, msg, target_method_vacuum_finish, NULL, &j);
        if (r < 0)
                return r;
